...
三个依赖。不需要独显（LLM 走 API），不需要数据库（SQLite 是 Python 自带的）。
...
丢后台不用管了。每 30 秒截一次屏，**屏幕没变化自动跳过**（锁屏/待机恢复不会爆发一堆重复截图）。
...
跑完后会：
1. 加载今天所有截图 → 去重过滤静止画面
2. LLM 一张张看截图，分析你在干什么
3. 把同类活动合并成时间段
4. 生成统计图表 + AI 幽默点评
5. 存入 SQLite（方便长期查询）
6. 问你要不要删截图 → 输入 `y` 一键清理
...
- **没有手机端**
- **LLM 分析需要联网**
- 截图多的时候分析可能要几分钟（每张 ~0.3s，去重后几百张 ≈ 几分钟）
